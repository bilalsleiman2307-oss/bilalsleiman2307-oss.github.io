...
const version = "trust-redesign-12";
...
const descriptionOverrides = {
  ...
  "leistung/schlüsselnotdienst": `Schlüsselnotdienst Berlin bei akuter Aussperrung: Türöffnung 24/7, Berechtigungsprüfung und klare Preisabsprache vor Beginn. Jetzt anrufen oder per WhatsApp Kontakt aufnehmen.`,
...
const languageSwitcher = (current = "de", ariaLabel = "Sprache wählen") => `<div class="language-switcher" aria-label="${esc(ariaLabel)}">${alternateLanguages.map(([code, href, short, label]) => `<a href="${href}" lang="${code}" hreflang="${code}" title="${label}"${code === current ? ' aria-current="page"' : ""}>${short}</a>`).join('<span aria-hidden="true">|</span>')}</div>`;

function headerFor(config = null, slug = "") {
  if (!config) return `<header class="site-header"><div class="container-wide header-inner"><a class="brand" href="/" aria-label="Trust Schlüsseldienst Berlin – Startseite"><img src="/assets/logo.jpeg?v=${version}" alt="Trust Schlüsseldienst Berlin Logo"></a><button class="menu-toggle" type="button" aria-label="Menü öffnen" aria-expanded="false" aria-controls="main-nav">&#9776;</button><nav id="main-nav" class="main-nav"><a href="/">Startseite</a><a href="/leistung/">Leistungen</a><a href="/türöffnung-berlin-24h-notdienst/">Türöffnung</a><a href="/schlüsseldienst-berlin-preise/">Preise</a><a href="/schlüsseldienst-in-der-nähe/">Einsatzgebiete</a><a href="/ratgeber/">Ratgeber</a>${languageSwitcher("de")}</nav><div class="header-actions"><a class="button button-whatsapp" href="https://wa.me/493040563878">WhatsApp</a><a class="button button-primary" href="tel:${phone}">Jetzt anrufen</a></div></div></header>`;
  const pageUrl = `/${slug}/`;
  return `<header class="site-header"><div class="container-wide header-inner"><a class="brand" href="${pageUrl}" aria-label="Trust Schlüsseldienst Berlin"><img src="/assets/logo.jpeg?v=${version}" alt="Trust Schlüsseldienst Berlin"></a><button class="menu-toggle" type="button" aria-label="${esc(config.nav.menu)}" aria-expanded="false" aria-controls="main-nav">&#9776;</button><nav id="main-nav" class="main-nav"><a href="${pageUrl}">${esc(config.nav.home)}</a><a href="#situations">${esc(config.nav.situations)}</a><a href="#services">${esc(config.nav.services)}</a><a href="#process">${esc(config.nav.process)}</a><a href="#prices">${esc(config.nav.prices)}</a><a href="#faq">${esc(config.nav.faq)}</a>${languageSwitcher(config.hreflang, config.nav.language)}</nav><div class="header-actions"><a class="button button-whatsapp" href="https://wa.me/493040563878">WhatsApp</a><a class="button button-primary" href="tel:${phone}">${esc(config.nav.call)}</a></div></div></header>`;
}

function footerFor(config = null) {
  if (!config) return `<footer class="site-footer"><div class="container-wide"><div class="footer-grid footer-grid-compact"><div><img class="footer-logo" src="/assets/logo.jpeg?v=${version}" alt="Trust Schlüsseldienst Berlin"><div class="footer-title">Trust Schlüsseldienst Berlin</div><p>Türöffnung, Schloss- und Zylinderwechsel in Berlin – mit Preisabsprache vor Beginn.</p>${googleReviewBadge}</div><div><div class="footer-title">Schnellzugriff</div><a href="/schlüsseldienst-berlin-preise/">Preise</a><a href="/leistung/schlüsselnotdienst/">Schlüsselnotdienst</a><a href="/türöffnung-berlin-24h-notdienst/">Türöffnung Berlin</a><a href="/schlüsseldienst-in-der-nähe/">Einsatzgebiete</a><a href="/impressum/">Impressum</a></div><div><div class="footer-title">Kontakt</div><p>${streetAddress}<br>13355 Berlin</p><a href="tel:${phone}">${phoneDisplay}</a><a href="https://wa.me/493040563878">WhatsApp</a><a href="mailto:${email}">${email}</a></div></div><div class="footer-bottom"><span>© ${new Date().getFullYear()} Trust Schlüsseldienst Berlin</span></div></div></footer><div class="mobile-callbar"><a class="button button-whatsapp" href="https://wa.me/493040563878">WhatsApp</a><a class="button button-primary" href="tel:${phone}">Anrufen</a></div>`;
  return `<footer class="site-footer"><div class="container-wide"><div class="footer-grid footer-grid-compact"><div><img class="footer-logo" src="/assets/logo.jpeg?v=${version}" alt="Trust Schlüsseldienst Berlin"><div class="footer-title">Trust Schlüsseldienst Berlin</div><p>${esc(config.footerSummary)}</p></div><div><div class="footer-title">${esc(config.footerLinks.overview)}</div><a href="#situations">${esc(config.footerLinks.situations)}</a><a href="#services">${esc(config.footerLinks.services)}</a><a href="#prices">${esc(config.footerLinks.prices)}</a><a href="#faq">${esc(config.footerLinks.faq)}</a><a href="/impressum/">${esc(config.footerLinks.legal)}</a></div><div><div class="footer-title">${esc(config.footerLinks.contact)}</div><p>${streetAddress}<br>13355 Berlin</p><a href="tel:${phone}">${phoneDisplay}</a><a href="https://wa.me/493040563878">WhatsApp</a><a href="mailto:${email}">${email}</a></div></div><div class="footer-bottom"><span>© ${new Date().getFullYear()} Trust Schlüsseldienst Berlin</span></div></div></footer><div class="mobile-callbar"><a class="button button-whatsapp" href="https://wa.me/493040563878">WhatsApp</a><a class="button button-primary" href="tel:${phone}">${esc(config.footerLinks.call)}</a></div>`;
}
...
function homePageContent(faqs) {
  return `${commercialHero("Schlüsseldienst Berlin", "Trust Schlüsseldienst Berlin hilft bei Türöffnungen, Schloss- und Zylinderwechsel sowie akuten Schlüsselfällen. Vor Beginn klären wir Türzustand, Berechtigung, Preis und Vorgehen.")}<section class="seo-welcome"><div class="container"><h2>Direkte Hilfe für Ihre Türsituation</h2><p>Ob akuter Notfall oder geplante Sicherheitsarbeit: Wählen Sie die passende Leistung. So finden Sie schnell die Informationen, Preise und nächsten Schritte für Ihren konkreten Fall.</p></div></section><section><div class="container"><div class="grid-3 service-cards-clean"><a class="card service-card-clean" href="/türöffnung-berlin-24h-notdienst/"><h2>Türöffnung Berlin</h2><p>Informationen zur Öffnung von zugefallenen oder abgeschlossenen Türen, zu typischen Situationen und zum Ablauf vor Ort.</p><span class="card-button">Mehr erfahren</span></a><a class="card service-card-clean" href="/leistung/schlüsselnotdienst/"><h2>Schlüsselnotdienst Berlin</h2><p>24/7 Hilfe bei Aussperrung, Schlüsselverlust, innen steckendem oder abgebrochenem Schlüssel.</p><span class="card-button">Mehr erfahren</span></a><a class="card service-card-clean" href="/leistung/schlosswechsel-berlin-schlösser-schnell-sicher-wechseln/"><h2>Schloss- und Zylinderwechsel</h2><p>Austausch nach Defekt, Verlust oder Sicherheitsbedenken. Material und Aufwand werden vor der Arbeit abgestimmt.</p><span class="card-button">Mehr erfahren</span></a></div></div></section>${processSection}${prices()}${reviewsSection("")}<section><div class="container split"><div><h2>Einbruchschutz und Sicherheitstechnik</h2><p>Zylinder, Schutzbeschlag, Schloss und Schließblech sollten als System zur Tür passen. Wir prüfen die vorhandene Situation und besprechen geeignete Maßnahmen und Material vor der Montage.</p><a class="button button-secondary" href="/leistung/sicherheitstechnik-berlin-einbruchschutz-vom-profi/">Einbruchschutz Berlin</a></div><div class="card"><h3>Vor dem Anruf hilfreich</h3><ul class="mini-list"><li>Adresse und Berliner Bezirk</li><li>Tür zugefallen oder abgeschlossen?</li><li>Steckt ein Schlüssel von innen?</li><li>Ausweis oder andere Legitimation verfügbar?</li></ul></div></div></section>${districts()}${faqSection("", "Schlüsseldienst Berlin", faqs)}${contactCta}`;
}
...
const openGraphTags = ({ title, description, ogLocale }, url) => `<meta property="og:type" content="website"><meta property="og:site_name" content="Trust Schlüsseldienst Berlin"><meta property="og:title" content="${esc(title)}"><meta property="og:description" content="${esc(description)}"><meta property="og:url" content="${url}"><meta property="og:locale" content="${ogLocale}"><meta property="og:image" content="${site}/assets/images/hero-schloss.jpg"><meta name="twitter:card" content="summary_large_image">`;
...
function internationalSchema(slug, config) {
  ...
  const graph = [
    ...
    {"@type":"Service",name:config.h1,serviceType:config.h1,inLanguage:config.lang,provider:{"@id":`${site}/#localbusiness`},areaServed:{"@type":"AdministrativeArea",name:"Berlin"}},
