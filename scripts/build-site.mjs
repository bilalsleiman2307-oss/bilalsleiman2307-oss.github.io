...
function districts(prioritize = false) {
  ...
  const remaining = districtRoutes.filter((slug) => !priority.includes(slug));
  return `<section id="einsatzgebiete"><div class="container"><div class="section-intro"><span class="eyebrow">Lokaler Schlüsseldienst</span><h2>Wichtige Einsatzgebiete in Berlin</h2><p>Für diese sechs Bezirke finden Sie ausführliche Informationen zu Anfahrt, Türöffnung, Preisen und dem Ablauf vor Ort.</p></div><div class="area-links priority-area-links">${priority.map((slug) => `<a href="/${slug}/"><strong>${districtNames[slug.slice(16)]}</strong><span>24/7 Türöffnung ab 59 €</span></a>`).join("")}</div><div class="section-intro district-overview-intro"><h3>Weitere Berliner Einsatzgebiete</h3></div><div class="area-links">${remaining.map((slug) => `<a href="/${slug}/">${districtNames[slug.slice(16)]}</a>`).join("")}</div></div></section>`;
}
