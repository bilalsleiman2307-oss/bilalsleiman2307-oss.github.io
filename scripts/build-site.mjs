...
const version = "trust-redesign-14";
const homepageStyleVersion = "trust-redesign-16";
...
function footerFor(config = null) {
  ...
  return `<footer class="site-footer"><div class="container-wide"><div class="footer-grid footer-grid-compact"><div><img class="footer-logo" src="/assets/logo-trust-transparent.png?v=${version}" width="1346" height="440" alt="Trust Schlüsseldienst Berlin"><div class="footer-title">Trust Schlüsseldienst Berlin</div><p>${esc(config.footerSummary)}</p></div><div><div class="footer-title">${esc(config.footerLinks.overview)}</div><a href="#situations">${esc(config.footerLinks.situations)}</a><a href="#services">${esc(config.footerLinks.services)}</a><a href="#prices">${esc(config.footerLinks.prices)}</a><a href="#faq">${esc(config.footerLinks.faq)}</a><a href="/impressum/">${esc(config.footerLinks.legal)}</a></div><div><div class="footer-title">${esc(config.footerLinks.contact)}</div><p>${streetAddress}<br>13355 Berlin</p><a href="tel:${phone}">${phoneDisplay}</a><a href="https://wa.me/493040563878">WhatsApp</a><a href="mailto:${email}">${email}</a></div></div><div class="footer-bottom"><span>© ${new Date().getFullYear()} Trust Schlüsseldienst Berlin</span></div></div></footer><div class="mobile-callbar"><a class="button button-whatsapp" href="https://wa.me/493040563878">WhatsApp</a><a class="button button-primary" href="tel:${phone}">${esc(config.footerLinks.call)}</a></div>`;
}
...
function priorityDistrictContent(slug, page) {
  const hero = `<section class="page-hero page-hero-image district-hero"><div class="container"><div class="hero-copy"><h1>${esc(page.h1)}</h1><p class="district-trust-line">Türöffnung ab 59 € · 24/7 erreichbar · in 10–30 Minuten vor Ort</p><p>${esc(page.hero)}</p>${googleReviewBadge}<div class="hero-actions"><a class="button button-primary" href="tel:${phone}">Jetzt anrufen</a><a class="button button-whatsapp" href="https://wa.me/493040563878">WhatsApp</a><a class="button button-secondary" href="/schlüsseldienst-berlin-preise/">Preise ansehen</a></div></div></div></section>`;
  ...
  return `${hero}${orders[slug].map((key) => blocks[key]).join("")}${experience}${links}${faqSection(slug, `Schlüsseldienst ${page.name}`, page.faqs)}${contactCta}`;
}
...
function homePrices() {
  return `<section id="preise" class="section-soft home-price-section"><div class="container"><div class="home-price-card"><div class="section-intro clean-intro"><span class="eyebrow">Klare Preise inklusive Mehrwertsteuer</span><h2>Preise für Türöffnung und Zylinderwechsel</h2><p class="home-price-lead">Die Türöffnung einer nur zugefallenen, nicht abgeschlossenen Tür beginnt von 07 bis 20 Uhr bei 59 €. Preis und Anfahrt werden für den beschriebenen Auftrag vor der Anfahrt am Telefon vereinbart.</p></div><div class="price-table-wrap"><table><thead><tr><th>Leistung</th><th>07–20 Uhr</th><th>20–00 Uhr</th><th>00–07 Uhr</th></tr></thead><tbody><tr><td>Zugefallene Tür öffnen</td><td>59 €</td><td>79 €</td><td>99 €</td></tr><tr><td>Abgeschlossene Tür öffnen</td><td>89 €</td><td>109 €</td><td>129 €</td></tr><tr><td>Zylinderwechsel</td><td>89 €</td><td>109 €</td><td>129 €</td></tr></tbody></table></div><div class="mobile-price-list"><article><h3>Zugefallene Tür</h3><p><span>07–20 Uhr</span><strong>59 €</strong></p><p><span>20–00 Uhr</span><strong>79 €</strong></p><p><span>00–07 Uhr</span><strong>99 €</strong></p></article><article><h3>Abgeschlossene Tür</h3><p><span>07–20 Uhr</span><strong>89 €</strong></p><p><span>20–00 Uhr</span><strong>109 €</strong></p><p><span>00–07 Uhr</span><strong>129 €</strong></p></article><article><h3>Zylinderwechsel</h3><p><span>07–20 Uhr</span><strong>89 €</strong></p><p><span>20–00 Uhr</span><strong>109 €</strong></p><p><span>00–07 Uhr</span><strong>129 €</strong></p></article></div><div class="home-price-notes"><p>Bei einer abgeschlossenen Tür, einem defekten Schloss oder zusätzlichen Arbeiten gilt der vorher besprochene Preis für diesen Fall. Profilzylinder, Schlösser, Beschläge und sonstiges Material gehören nicht automatisch zum Türöffnungspreis.</p><p>Material wird nur nach ausdrücklicher Absprache eingesetzt und berechnet. Alle sichtbaren Preise enthalten die Mehrwertsteuer.</p><a class="button button-secondary" href="/schlüsseldienst-berlin-preise/">Vollständige Preisinformationen</a></div></div></div></section>`;
}

function homePageContent(faqs) {
  const heroRating = `<a class="home-hero-rating" href="${googleReviewUrl}" target="_blank" rel="noopener noreferrer" aria-label="Google-Bewertungen von Trust Schlüsseldienst Berlin ansehen"><span class="google-word" aria-hidden="true"><span>G</span><span>o</span><span>o</span><span>g</span><span>l</span><span>e</span></span><span class="review-stars" aria-hidden="true">★★★★★</span><span><strong>5,0</strong> bei 105 Bewertungen</span></a>`;
  const hero = `<section class="page-hero page-hero-image home-hero"><div class="container"><div class="hero-copy">${heroRating}<h1><span class="home-h1-brand">Trust Schlüsseldienst Berlin</span><span class="home-h1-offer"> – Türöffnung ab 59 €</span></h1><p class="home-hero-intro">Ausgesperrt, Tür zugefallen, Schlüssel verloren oder im Schloss abgebrochen? Trust Schlüsseldienst Berlin ist 24 Stunden erreichbar und hilft bei Türöffnungen in Berlin. Den Festpreis und die Anfahrt vereinbaren wir bereits am Telefon. Abhängig von Einsatzadresse und Verkehr sind wir in 10–30 Minuten vor Ort.</p><ul class="home-hero-checks"><li>24/7 in Berlin erreichbar</li><li>Türöffnung einer zugefallenen Tür ab 59 €</li><li>Festpreis und Anfahrt vorab am Telefon</li><li>10–30 Minuten je nach Adresse und Verkehr</li></ul><div class="hero-actions home-hero-actions"><a class="button button-primary" href="tel:${phone}">Jetzt anrufen · 030 40563878</a><a class="button button-whatsapp" href="https://wa.me/493040563878">WhatsApp-Kontakt</a></div></div></div></section>`;

  const situations = `<section id="türsituationen"><div class="container"><div class="section-intro clean-intro"><span class="eyebrow">Notfall richtig beschreiben</span><h2>Schnelle Hilfe, wenn die Tür zu ist</h2><p>Nennen Sie beim Anruf Adresse, Bezirk, Uhrzeit und Türzustand. Wichtig sind ein innen steckender Schlüssel und erkennbare Schäden an Schloss, Zylinder, Beschlag oder Tür.</p></div><div class="grid-3 home-situation-grid"><article class="card service-card-clean"><h3>Tür nur zugefallen oder ausgesperrt</h3><p>Die Tür ist ins Schloss gefallen, wurde aber nicht verriegelt. Für diese Situation gilt die <a href="/leistung/öffnung-bei-zugefallenen-türen/">Öffnung einer zugefallenen Tür</a>; einen Überblick bietet auch die Seite <a href="/türöffnung-berlin-24h-notdienst/">Türöffnung Berlin</a>.</p></article><article class="card service-card-clean"><h3>Schlüssel steckt innen</h3><p>Steckt der Schlüssel auf der Innenseite, teilen Sie uns mit, ob außen ein Schlüsselloch vorhanden ist und ob die Tür verriegelt wurde. Lesen Sie dazu die Hinweise für den Fall <a href="/schlüssel-steckt-innen-tür-zu/">Schlüssel steckt innen, Tür zu</a>.</p></article><article class="card service-card-clean"><h3>Schlüssel verloren oder abgebrochen</h3><p>Bei einem verlorenen Schlüssel prüfen wir, ob eine Türöffnung genügt oder ein Sicherheitsrisiko besteht. Ist der Schlüssel im Zylinder abgebrochen, hilft der <a href="/leistung/schlüsselnotdienst/#schluessel-abgebrochen">Schlüsselnotdienst bei einem abgebrochenen Schlüssel</a>.</p></article><article class="card service-card-clean"><h3>Tür abgeschlossen</h3><p>Eine verriegelte Tür erfordert eine andere Prüfung als eine nur zugefallene Tür. Auf der Seite <a href="/leistung/öffnung-bei-abgeschlossenen-türen/">abgeschlossene Tür öffnen</a> erklären wir, warum Methode und Preis vom vorhandenen Schloss abhängen.</p></article><article class="card service-card-clean"><h3>Schloss oder Zylinder defekt</h3><p>Dreht der Schlüssel durch, klemmt der Zylinder oder lässt sich das Schloss nicht mehr bedienen, beschreiben Sie das genaue Verhalten. Ein Austausch wird erst empfohlen und ausgeführt, wenn der Zustand geprüft und der Preis vereinbart wurde.</p></article><article class="card service-card-clean"><h3>Akuter Schlüsselnotdienst</h3><p>Bei einer Aussperrung erreichen Sie den <a href="/leistung/schlüsselnotdienst/">Schlüsselnotdienst Berlin</a> täglich rund um die Uhr. Am Telefon klären wir zuerst Türzustand, Berechtigung, Anfahrt und den Preis des konkreten Auftrags.</p></article></div></div></section>`;

  const process = `<section class="section-blue" id="ablauf"><div class="container"><div class="section-intro"><span class="eyebrow">Nachvollziehbarer Auftrag</span><h2>So läuft die Türöffnung ab</h2></div><div class="process-grid"><article class="process-step"><span>1</span><h3>Situation und Adresse schildern</h3><p>Nennen Sie Einsatzadresse, Bezirk, Uhrzeit und ob die Tür zugefallen, abgeschlossen oder beschädigt ist.</p></article><article class="process-step"><span>2</span><h3>Festpreis und Anfahrt vereinbaren</h3><p>Der Festpreis für den beschriebenen Auftrag und die Anfahrt werden am Telefon vor der Anfahrt vereinbart.</p></article><article class="process-step"><span>3</span><h3>Berechtigung prüfen</h3><p>Die Prüfung erfolgt durch Ausweis vor oder direkt nach der Öffnung. Liegt der Ausweis in der Wohnung, kann alternativ zunächst ein Nachbar die Berechtigung bestätigen.</p></article><article class="process-step"><span>4</span><h3>Tür öffnen und Arbeit abschließen</h3><p>Die Tür wird entsprechend ihrem Zustand geöffnet. Zusätzliche Arbeiten oder Material erfolgen nur nach erneuter Absprache.</p></article></div></div></section>`;
  ...
  return `${hero}${homePrices()}${situations}${process}${services}${damageFree}${districts(true)}${reasons}${reviewsSection("")}${security}${faqSection("", "Schlüsseldienst Berlin", faqs)}${contactCta}`;
}
...
function internationalPrices(config) {
  ...
  return `<section id="prices" class="section-soft"><div class="container"><div class="section-intro"><span class="eyebrow">${esc(config.priceTable.eyebrow)}</span><h2>${esc(config.priceTitle)}</h2><p>${esc(config.priceIntro)}</p></div><div class="price-table-wrap"><table><thead><tr><th>${esc(serviceLabel)}</th>${timeLabels.map((label) => `<th>${esc(label)}</th>`).join("")}</tr></thead><tbody>${desktopRows}</tbody></table></div><div class="mobile-price-list">${mobileRows}</div><p class="fine-print">${esc(config.priceTable.finePrint)}</p></div></section>`;
}

function internationalContent(config) {
  return `<section class="page-hero page-hero-image international-hero"><div class="container"><div class="hero-copy"><h1>${esc(config.h1)}</h1><p class="international-hero-lead">${esc(config.hero)}</p><a class="hero-phone" href="tel:${phone}">${phoneDisplay}</a><div class="hero-actions"><a class="button button-primary" href="tel:${phone}">${esc(config.nav.call)}</a><a class="button button-whatsapp" href="https://wa.me/493040563878">WhatsApp</a></div><ul class="hero-points"><li>24/7</li><li>10–30 min</li><li>${config.hreflang === "en" ? "From €59" : config.hreflang === "es" ? "Desde 59 €" : "A partir de 59 €"}</li><li>Berlin</li></ul></div></div></section><section class="international-trust"><div class="container"><div class="section-intro clean-intro"><h2>${esc(config.trustTitle)}</h2><p>${esc(config.trustIntro)}</p></div><div class="grid-3 service-cards-clean">${localizedCards(config.trust)}</div></div></section><section id="situations" class="section-soft"><div class="container"><div class="section-intro clean-intro"><h2>${esc(config.situationsTitle)}</h2><p>${esc(config.situationsIntro)}</p></div><div class="grid-3 service-cards-clean">${localizedCards(config.situations)}</div></div></section><section id="services"><div class="container"><div class="section-intro clean-intro"><h2>${esc(config.servicesTitle)}</h2><p>${esc(config.servicesIntro)}</p></div><div class="grid-3 service-cards-clean">${localizedCards(config.services)}</div></div></section><section id="process" class="section-blue"><div class="container"><div class="section-intro"><h2>${esc(config.processTitle)}</h2></div><div class="process-grid international-process">${config.process.map(([heading, text], index) => `<div class="process-step"><span>${index + 1}</span><h3>${esc(heading)}</h3><p>${esc(text)}</p></div>`).join("")}</div></div></section><section><div class="container split"><div><h2>${esc(config.areasTitle)}</h2><p>${esc(config.areas)}</p></div><div class="card international-area-card"><strong>Berlin</strong><p>Gesundbrunnen · Wedding · Mitte · Pankow · Prenzlauer Berg · Reinickendorf</p></div></div></section>${internationalPrices(config)}<section id="faq"><div class="container"><div class="section-intro clean-intro"><h2>${esc(config.faqTitle)}</h2></div><div class="faq-list">${config.faqs.map(([question, answer]) => `<details class="faq-item"><summary>${esc(question)}</summary><p>${esc(answer)}</p></details>`).join("")}</div></div></section><section><div class="container"><div class="cta-panel"><div><h2>${esc(config.ctaTitle)}</h2><p>${esc(config.ctaText)}</p></div><div class="cta-actions"><a class="button button-primary" href="tel:${phone}">${phoneDisplay}</a><a class="button button-whatsapp" href="https://wa.me/493040563878">WhatsApp</a></div></div></div></section>`;
}
